...
public class WeiboLoginServiceImpl implements IWeiboLogin {
  public static Logger logger = LoggerFactory.getLogger(WeiboLoginServiceImpl.class);
  //用户名base64
  private String su;
   ...
   * 
   * @Title: encodePwd   
   * @Description: 密码进行RSA加密
   * @param: @return      
   * @return: boolean      
   * @throws
   */
  private boolean encodePwd(String password) {
      ScriptEngineManager sem = new ScriptEngineManager();
      ScriptEngine se = sem.getEngineByName("javascript");
      try {
          // 使用js加密密码,RSA,调用js内方法 ，如下面注释部分。
          se.eval(sina_js);
          //调用js内部函数用于加密
          if (se instanceof Invocable) {
              Invocable iv = (Invocable) se;
              sp = (String) iv.invokeFunction("getpass", password, this.servertime, this.nonce,
                      this.pubkey);
          }
          return true;
